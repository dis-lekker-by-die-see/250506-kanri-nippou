...
// const ttf_font: string = "ipaexg.ttf";
// const base64_font: string = "ipaexgbase64.txt";
// let fontReady: boolean = false;
// let base64Font: string = "";
// // Load the font on page load
// window.onload = async () => {
//   try {
//     const res: Response = await fetch(base64_font);
...
//     fontReady = true;
//     (document.getElementById("downloadBtn") as HTMLButtonElement).disabled =
//       false;
//   } catch (e) {
//     console.error(`Failed to load ${base64_font}:`, e);
//     try {
//       const ttfRes: Response = await fetch(ttf_font);
//       if (!ttfRes.ok) throw new Error(`Fetch TTF failed: ${ttfRes.statusText}`);
//       const fontData: ArrayBuffer = await ttfRes.arrayBuffer();
...
//     const table1Inputs = document.querySelectorAll(
//       "table:nth-child(2) input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table1"] = {
//       資料No: table1Inputs[0].value,
//       製品名: table1Inputs[1].value,
...
//     const table2Inputs = document.querySelectorAll(
//       "table.weight-row input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table2"] = {
//       設定重量: table2Inputs[0].value,
//       PPバント_青_承認: table2Inputs[1].value,
...
//     const table3Inputs = document.querySelectorAll(
//       "table:nth-child(4) input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table3"] = {
//       注意事項: [
//         {
...
//     const table4Inputs = document.querySelectorAll(
//       "table:nth-child(5) input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table4"] = {
//       段ボール: {
//         時間: table4Inputs[0].value,
...
//     const table5Inputs = document.querySelectorAll(
//       "table:nth-child(7) input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table5"] = {
//       出来高_1: Array.from(table5Inputs)
//         .slice(0, 14)
...
//     const table6Inputs = document.querySelectorAll(
//       "table:nth-child(8) input.form-input"
//     ) as NodeListOf<HTMLInputElement>;
//     tableData["table6"] = {
//       ケースNo: Array.from(table6Inputs)
//         .slice(0, 14)
...
//     const jsPDF: any = (window as any).jspdf.jsPDF; // Treat jsPDF as any to bypass type issues
...
//       doc.setFontSize(10);
//       doc.text(
//         `製造日: ${formData.dates.manufacturing.year}-${formData.dates.manufacturing.month}-${formData.dates.manufacturing.day} | 包装日: ${formData.dates.packaging.year}-${formData.dates.packaging.month}-${formData.dates.packaging.day}`,
//         297.64,
//         35,
//         { align: "center" }
//       );
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    downloadBtn.addEventListener("click", async () => {
        ...
        // // Table 2: 設定重量, PPバント（青） (2 inputs + 1 static value)
        // const table2Inputs = document.querySelectorAll(
        //   "table.weight-row input.form-input"
        // ) as NodeListOf<HTMLInputElement>;
        // const ppBandCell = document.querySelector(
        //   "table.weight-row tr:nth-child(2) td:nth-child(2)"
        // ) as HTMLTableCellElement;
        // tableData["table2"] = {
        //   設定重量: table2Inputs[0].value,
        //   PPバント_青: ppBandCell.textContent?.trim() || "", // Fetch static value from table cell
        //   承認: table2Inputs[1].value,
        //   照査: table2Inputs[2].value,
        // };
        // Table 2: 設定重量, PPバント（青） (3 inputs)
        ...
        // const jsonFilename: string = `form_data_${timestamp}.json`;
        const jsonFilename = `${formData.tables.table1.資料No}_${timestamp}.json`;
        // Download JSON
        ...
        doc.autoTable({
            startY: lastY,
            head: [["設定重量", "PPバント（青）", "承認", "照査"]],
            body: [
                [
                    formData.tables.table2.設定重量,
                    formData.tables.table2.PPバント_青,
                    formData.tables.table2.承認,
                    formData.tables.table2.照査,
                ],
            ],
            theme: "grid",
            headStyles: {
        ...
        // Download PDF
        const pdfFilename = `${formData.tables.table1.資料No}_${timestamp}.pdf`;
        doc.save(pdfFilename);
    });
});
